...
import { authStates as AuthStates, roles } from '../../enums/lib'

// Control Personas
import {
  ResetAllData as ResetMenteeData,
  GetMenteeFromPersona,
} from '../mentee/actions'
import {
  ResetAllData as ResetMentorData,
  GetMentorFromPersona,
} from '../mentor/actions'
import {
  ResetAllData as ResetMemberData,
  StoreMember,
} from '../member/actions'
...
const GetUserPersona = () => async (dispatch, getState) => {
  try {
    ...
    const persona = (await API.graphql(graphqlOperation(getPersona, { id: session.id }))).data.getPersona
...
export const UpdateActivePersona = newPersona => async (dispatch, getState) => {
  try {
    ...
    const persona = (
      await API.graphql(
        graphqlOperation(updatePersona, {
          input: {
            id: session.id,
            activePersona: newPersona, // Deprecated; remove
...
export const SetAuthSession = updates => (dispatch, getState) => {
  try {
    const { personaReducer: { persona, session } } = getState()
    const { user, state, error } = { ...session, ...updates }

    const userId = getUserId(user)
    const userGroups = getUserGroups(user)

    // if signing out, clear the state and bail
    if (state === AuthStates.signIn) {
      dispatch(StoreSession({ user: null, state, error, id: userId, groups: userGroups }))
    ...
    dispatch(StoreSession({ user, state, error, id: userId, groups: userGroups }))
...
export const SyncCognitoData = (bypassCache = true, callback) => async dispatch => {
  try {
    ...
    // Then download latest User data. We do this after updating the Session so that we have permission
    const refreshUser = await Auth.currentAuthenticatedUser({ bypassCache })
...
export const SignUp = () => async (dispatch, getState) => {
  try {
    ...
    dispatch(StoreSession({ ...session, user }))
    return user
...
export const ChangePassword = () => async (dispatch, getState) => {
  try {
    ...
    const { user: updatedUser } = await Auth.changePassword(currentUser, oldPassword, password)

    await dispatch(StoreSession({ ...session, user: updatedUser }))
...
export const SignIn = (emailFallback = '', passwordFallback = '') => async (dispatch, getState) => {
  ...
  const shouldBlockAttempt = (failedSignInAttempts = []) => {
    ...
    const mostRecentAttempt = sortedAttempts[sortedAttempts.length - 1] || { time: Date.now(), delay: -1 }
  ...
  if (!shouldBlockAttempt(!!authCache && authCache.fails || undefined)) {
    try {
      ...
      */
      ...
      console.log({ attempt: 'one', failureReason: error.message })
      if (error.code === 'UserNotConfirmedException') {
        // User needs to confirm email, so resend a code and save the email for any UI usages
        dispatch(StoreSession({ ...session, username: email || emailFallback }))
        ...
        if (!authCache || !authCache.shouldSkipRetryWithCasing) {
          ...
          */
          try {
            ...
            console.log({ attempt: 'two', failureReason: error.message })
            if (error.code === 'UserNotConfirmedException') {
              // User needs to confirm email, so resend a code and save the email for any UI usages
              dispatch(StoreSession({ ...session, username: email || emailFallback }))
              dispatch(RequestMFAVerificationCode(true))
    ...
    dispatch(LogError(null, `Too many sign-in attempts, please wait ${getSecondsRemaining(!!authCache && authCache.fails)}s before retrying`))
    // If user failing continuously on an email that doesn't exist, let's turn on the casing retry once more
    dispatch(UpdateAuthCache({
      ...authCache,
...
export const SignOut = () => dispatch => {
  try {
    ...
    // No need to reset Persona model data; handled by SyncCognitoData signout callback
...
export const SubmitMFAVerificationCode = (mfaAttempt, shouldUseCognito = false) => async (dispatch, getState) => {
  try {
    const {
      personaReducer: { session, persona, email, password },
    } = getState()

    let response = null

    if (shouldUseCognito) {
      response = await Auth.confirmSignUp(session.user.username, mfaAttempt)
      // Automatically sign the user in with their credentials from creating their account
...
export const RequestMFAVerificationCode = (shouldUseCognito = false) => async (dispatch, getState) => {
  try {
    ...
    if (shouldUseCognito) {
      ...
      successfullySentEmail = await API.post(
        'actionsApi',
        `/send-persona-confirmation-email`
      )
    }
...
export const SelectMentor = () => async (dispatch, getState) => {
  try {
    const {
      mentorReducer: { newMentor },
    } = getState()
    
    dispatch(TrackAction('selectingMentorPersona'))
    dispatch(SetPersonaLoading(true))
    const updatedPersona = await API.post(
      'actionsApi',
      `/request-to-join-mentor-group`,
      {
        body: {
